...
import { DEFAULT_MATERIAL_TUNE, launchTelemetry, type ChapterId, type ConfigState } from "./audiRs5Data";
...
const CAMERAS: Record<ChapterId, CamPose> = {
  ...
  specs: {
    // Same distance as side packshot; yaw only (~48° from +X) — more face-on.
    // Right-half composition via setViewOffset (text stays clear).
    pos: new THREE.Vector3(5.35, 1.72, 5.95),
    target: new THREE.Vector3(0.05, 0.52, -0.2),
    up: DEFAULT_UP.clone(),
    fov: 29,
  },
  exterior: {
...
function buildDistanceStrip() {
  ...
  // Fixed above the rail — does not follow launch progress
  liveSpr.position.set(STRIP_X + 0.42, STRIP_Y + 0.32, zAt(0) + 0.4);
  liveSpr.renderOrder = 7;
  group.add(liveSpr);

  const paintLive = (speedKmh: number, nm: number, rpm: number, heat: number) => {
    ...
    liveCtx.fillText(`${speedKmh} km/h`, 8, 28);
  ...
  const setProgress = (t: number) => {
    ...
    const { u, distFrac, speedKmh, torqueNm, rpm } = telem;
    ...
    paintLive(speedKmh, torqueNm, rpm, heat);
  };
...
export function AudiRs5Canvas({ state, className, onReady, onProgress }: Props) {
  ...
  useEffect(() => {
    ...
    renderer.toneMappingExposure = DEFAULT_MATERIAL_TUNE.exposure;
    ...
    // Softbox packshot + studio HDRI (pre-crash look, slightly dialed for GPU)
    const key = new THREE.RectAreaLight(0xfff6ee, DEFAULT_MATERIAL_TUNE.keyIntensity, 5.5, 3.2);
    key.position.set(3.8, 5.2, 3.4);
    key.lookAt(0, 0.6, 0);
    scene.add(key);

    const rimBox = new THREE.RectAreaLight(0xc8d4e0, DEFAULT_MATERIAL_TUNE.rimIntensity, 3.5, 2.4);
    rimBox.position.set(-4.2, 3.4, -3.2);
    rimBox.lookAt(0, 0.7, 0);
    scene.add(rimBox);

    const fill = new THREE.HemisphereLight(0xd8e4dc, 0x101010, DEFAULT_MATERIAL_TUNE.fillIntensity);
    scene.add(fill);

    // Contact-shadow key — target must be in the scene or the map never lands on the catcher
    const keyShadow = new THREE.DirectionalLight(0xfff4ea, DEFAULT_MATERIAL_TUNE.shadowIntensity);
    ...
    keyShadow.target.position.set(0, 0.2, 0);
    scene.add(keyShadow);
    scene.add(keyShadow.target);

    // Dual floor: faint matte disk (needed so ShadowMaterial can darken) + catcher
    ...
    // Spoke edges need higher MSAA; re-apply after every composer.setSize (it resets samples).
    const composer = new EffectComposer(renderer);
    const msaaSamples = Math.min(8, renderer.capabilities.maxSamples || 4);
    const applyComposerMsaa = () => {
    ...
    const bloomPass = new UnrealBloomPass(
      ...
      DEFAULT_MATERIAL_TUNE.bloomStrength,
      DEFAULT_MATERIAL_TUNE.bloomRadius,
      DEFAULT_MATERIAL_TUNE.bloomThreshold,
    );
    ...
    void (async () => {
      ...
      // Dim Kloppenheim — warm softbox fill without blowing lacquer
      scene.environmentIntensity = DEFAULT_MATERIAL_TUNE.environmentIntensity;
      scene.background = new THREE.Color(0x101010);
    ...
    const SPECS_BIAS_FRAC = 0.58;
    let frustumBiasFrac = stateRef.current.chapter === "specs" ? SPECS_BIAS_FRAC : 0;

    const applyFrustumBias = (biasFrac: number) => {
      const w = mount.clientWidth || 1;
      const h = mount.clientHeight || 1;
      const bias = Math.max(0, Math.round(w * biasFrac));
      if (bias <= 0) {
        if (camera.view?.enabled) camera.clearViewOffset();
        camera.aspect = w / h;
        camera.updateProjectionMatrix();
      } else {
        // Wider virtual film; left slice → car sits in the right half (sheet stays clear).
        camera.setViewOffset(w + bias, h, 0, 0, w, h);
      }
    };
    ...
    const resize = () => {
      ...
      applyFrustumBias(frustumBiasFrac);
    ...
    const tick = (now = performance.now()) => {
      ...
      // Animate composition shift with the camera rail — no snap/teleport.
      const biasTarget = st.chapter === "specs" ? SPECS_BIAS_FRAC : 0;
      frustumBiasFrac = st.reducedMotion
        ? biasTarget
        : THREE.MathUtils.damp(frustumBiasFrac, biasTarget, 4.2, dt);
      if (Math.abs(frustumBiasFrac - biasTarget) < 0.0005) frustumBiasFrac = biasTarget;
      ...
      // After pose update so projection keeps the animated composition shift.
      applyFrustumBias(frustumBiasFrac);

      const tune = DEFAULT_MATERIAL_TUNE;
      const keyTarget = cabinMode ? 2.6 : tune.keyIntensity + launchEnergy * 0.45;
      const rimTarget = cabinMode ? 1.4 : tune.rimIntensity;
      const fillTarget = cabinMode ? 0.22 : tune.fillIntensity;
      const shadowTarget = cabinMode ? 0.28 : tune.shadowIntensity;
      const exposureTarget = cabinMode ? 0.92 : tune.exposure;
      const bloomTarget = cabinMode ? 0.02 : tune.bloomStrength + launchEnergy * 0.015;

      key.intensity = THREE.MathUtils.lerp(key.intensity, keyTarget, 0.08);
      rimBox.intensity = THREE.MathUtils.lerp(rimBox.intensity, rimTarget, 0.08);
      fill.intensity = THREE.MathUtils.lerp(fill.intensity, fillTarget, 0.08);
      keyShadow.intensity = THREE.MathUtils.lerp(keyShadow.intensity, shadowTarget, 0.08);
      renderer.toneMappingExposure = THREE.MathUtils.lerp(renderer.toneMappingExposure, exposureTarget, 0.08);
      ...
      if (scene.environment) scene.environmentIntensity = tune.environmentIntensity;

      const showGround = st.chapter !== "quattro";
      matteFloor.visible = showGround;
      catcher.visible = showGround;
      catcherMat.opacity = st.chapter === "quattro" ? 0.18 : 0.52;
      ...
      if (handle) {
        const cfg = st.config;
        if (applied.materialTune !== "default") {
          handle.applyMaterialTune(tune);
          handle.setEnvIntensity(cabinMode ? 0.8 : tune.envMeshIntensity);
          applied.materialTune = "default";
        }
        ...
        const stageZ = st.chapter === "hero" ? -0.55 : 0;
        ...
        // Ground plane = tire contact, NOT root origin (root.y is ~half car height).
        const groundY = stageY - modelBaseY;
        matteFloor.position.y = THREE.MathUtils.lerp(matteFloor.position.y, groundY - 0.002, 0.1);
        catcher.position.y = THREE.MathUtils.lerp(catcher.position.y, groundY + 0.001, 0.1);
        ...
        distanceStrip.group.position.y = THREE.MathUtils.lerp(distanceStrip.group.position.y, groundY, 0.1);
        keyShadow.target.position.set(0, groundY + 0.35, stageZ);
        keyShadow.target.updateMatrixWorld();
