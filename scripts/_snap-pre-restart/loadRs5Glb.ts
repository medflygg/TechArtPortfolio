...
function applyGlassPbr(
  ...
  // (Transmission + short attenuation looked solid black/plastic here;
  // light color + alpha was the earlier milky look.)
  if (kind === "tail") {
  ...
  phys.envMapIntensity = kind === "body" ? 2.05 : 1.55;
...
function applyChromePbr(mat: THREE.MeshStandardMaterial, maps: BodyPbrMaps, env: THREE.Texture | null) {
  mat.color.set(0xc8c6c0);
  mat.metalness = 1;
  mat.roughness = 0.26;
  mat.normalMap = maps.brushedNormal;
  mat.normalScale = new THREE.Vector2(0.22, 0.1);
...
function applyAlloyPbr(mat: THREE.MeshStandardMaterial, maps: BodyPbrMaps, env: THREE.Texture | null) {
  ...
  mat.roughness = 0.28;
  // Always add brushed micro-detail — baked MR alone reads as flat paint
  mat.normalMap = maps.brushedNormal;
  mat.normalScale = new THREE.Vector2(0.38, 0.16);
  mat.roughnessMap = maps.brushedRough;
...
function applySilverPbr(mat: THREE.MeshStandardMaterial, maps: BodyPbrMaps, env: THREE.Texture | null) {
  mat.color.set(0xbdbbb4);
  mat.metalness = 0.9;
  mat.roughness = 0.3;
  if (!mat.normalMap) {
    mat.normalMap = maps.brushedNormal;
    mat.normalScale = new THREE.Vector2(0.18, 0.1);
  }
  if (!mat.roughnessMap) mat.roughnessMap = maps.brushedRough;
  ...
  root.traverse((obj) => {
    ...
    for (const m of asStd(obj.material)) {
      ...
      if (n.includes("matte_black") || n.includes("matal_black") || n.includes("rs5_texture")) {
        ...
        m.envMapIntensity = n.includes("tail_cover") || n.includes("hl_cover") ? 1.2 : 1.45;
      } else if (n.includes("black_palastic") || n.includes("tail_plastic")) {
  ...
  return new Promise((resolve, reject) => {
    loader.load(
      ...
      (gltf) => {
        ...
        if (opts.envMap) applyEnv(root, opts.envMap, 1.15);
        ...
        const setPaint = (id: PaintId) => {
          ...
          for (const m of mats.paint) {
            ...
            m.specularIntensity = p.kind === "solid" ? 1 : 0.85;
        ...
        const setWheel = (id: WheelId) => {
          ...
          tint(mats.wheel, w.hex, { metalness: 0.94, roughness: id === "bronze" ? 0.34 : 0.2 });
          for (const m of mats.wheel) {
            applyAlloyPbr(m, maps, opts.envMap ?? null);
            m.color.set(w.hex);
            m.roughness = id === "bronze" ? 0.34 : 0.2;
            m.needsUpdate = true;
          }
        };
